...
public class TownsManager {
    ...
    // Сохранение города с координатами (перед сохранением списка нужно очистить старый)
    public void saveTown(String townName, double latitude, double longitude, String sunrise, String sunset, boolean favorite) {
        try {
            if (!checkTown(townName)) {
                new TownsTable(townName, latitude, longitude, sunrise, sunset, favorite).save();
            }
        } catch (Exception e) {
            Log.i(mTag, e.getMessage());
        }
